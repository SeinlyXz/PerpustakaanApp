...
namespace PerpustakaanAppMVC.Model.Repository
{
    internal class MahasiswaRepository
    {
        ...
        public int Update(Mahasiswa mhs)
        {
            ...
            Console.WriteLine(mhs.Npm);
            Console.WriteLine(mhs.Nama);
            Console.WriteLine(mhs.Angkatan);
            // membuat objek command menggunakan blok using
            using (SQLiteCommand cmd = new SQLiteCommand(sql, _conn))
            {
                // mendaftarkan parameter dan mengeset nilainya
        ...
        public List<Mahasiswa> ReadAll()
        {
            ...
            {
                // deklarasi perintah SQL
                string sql = @"select npm, nama, angkatan from mahasiswa order by npm";
                // membuat objek command menggunakan blok using
                using (SQLiteCommand cmd = new SQLiteCommand(sql, _conn))
                {
                    ...
                    using (SQLiteDataReader dtr = cmd.ExecuteReader())
                    {
                        ...
                        while (dtr.Read())
                        {
                            ...
                            {
                                Npm = dtr["npm"].ToString(),
                                Nama = dtr["nama"].ToString(),
                                Angkatan = dtr["angkatan"].ToString()
